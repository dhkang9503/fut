...
- Timeframes (priority): 5m → 3m → 15m → 30m → 1h
- Patterns:
  • Triangles / Wedges (symmetrical, ascending, descending, rising/falling wedge)
  • Box-range breakout + volume confirmation
  • Double Top / Double Bottom (M/W) with neckline break
  • Head & Shoulders / Inverse H&S with neckline break
- Anti-fake: confirm bars; body≥ATR*k; body/range ratio
- Retest entries with SL/TP and leverage suggestion (SL at ~10% notional loss)
- Telegram: TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID from environment; start alert to Telegram
...
class Config:
    ...
    # Box-range options
    box_window: int = 80            # bars to define range
    box_flat_tol_pct: float = 0.004 # tolerate highs/lows variance to consider "flat-ish"
    box_break_buffer_pct: float = 0.001

    # Double top/bottom (M/W)
    dbl_pivot_min_gap: int = 5
    dbl_price_tolerance: float = 0.004  # 0.4% tolerance between twin peaks/troughs
...
def classify_triangle(m_up: float, m_lo: float) -> Optional[str]:
    up, lo = m_up, m_lo
    if "symmetrical" in cfg.patterns_tri and (up < 0 and lo > 0):
        return "triangle_sym"
    if "ascending" in cfg.patterns_tri and slope_is_flat(up) and lo > 0:
        return "triangle_asc"
    if "descending" in cfg.patterns_tri and up < 0 and slope_is_flat(lo):
        return "triangle_desc"
    if "rising_wedge" in cfg.patterns_tri and (up > 0 and lo > 0 and up < lo):
        return "wedge_rising"
    if "falling_wedge" in cfg.patterns_tri and (up < 0 and lo < 0 and up > lo):
...
def tri_contraction_ok(candles: List[Candle], tri: TriangleState, atr_recent: float, atr_vals: List[float]) -> bool:
    ...
    if len(window) < cfg.atr_window + 10:
        return False
    cut = len(window) // 2
    early = window[:cut]
    late  = window[cut:]
    ...
    if early_range <= 0:
        return False
    if (late_range / early_range) > cfg.min_contraction_ratio:
        return False
    if len(atr_vals) < cfg.atr_window:
        return False
    atr_long = float(np.nanmean(atr_vals[max(0, tri.start_idx - cfg.atr_window):tri.end_idx + 1]))
    if not (atr_recent > 0 and atr_long > 0):
        return False
    if (atr_recent / atr_long) > cfg.atr_contraction_factor:
        return False
    return True
...
def detect_box(candles: List[Candle]) -> Optional[Tuple[float, float]]:
    """Return (range_high, range_low) if recent window is boxy/flat; else None."""
    if len(candles) < cfg.box_window + 5:
        return None
    ...
    rh = max(highs)
    rl = min(lows)
    tol_h = rh * cfg.box_flat_tol_pct
    tol_l = rl * cfg.box_flat_tol_pct
...
# --------------------------- M/W (Double Top/Bottom) --------------------------- #

def find_recent_pivots(candles: List[Candle], span: int, limit: int = 60):
    """Collect recent pivot highs/lows indices within 'limit' bars."""
...
def detect_double_top(candles: List[Candle]) -> Optional[Dict]:
    ...
    if len(ph) < 2 or len(pl) < 1:
        return None
    h1, h2 = ph[-2], ph[-1]
    if h2 - h1 < cfg.dbl_pivot_min_gap:
        return None
    p1 = candles[h1].high
    p2 = candles[h2].high
    if abs(p1 - p2) / ((p1 + p2) / 2) > cfg.dbl_price_tolerance:
        return None
    # neckline is the lowest low between h1 and h2
...
def detect_double_bottom(candles: List[Candle]) -> Optional[Dict]:
    ...
    if len(pl) < 2 or len(ph) < 1:
        return None
    l1, l2 = pl[-2], pl[-1]
    if l2 - l1 < cfg.dbl_pivot_min_gap:
        return None
    p1 = candles[l1].low
    p2 = candles[l2].low
    if abs(p1 - p2) / ((p1 + p2) / 2) > cfg.dbl_price_tolerance:
        return None
    # neckline is the highest high between l1 and l2
...
def detect_head_shoulders(candles: List[Candle]) -> Optional[Dict]:
    """Heuristic: last ~100 bars, find three pivot highs: L, H, R with H highest.
       Shoulders (L,R) within tolerance; neckline from pivot lows between them."""
    ph, pl = find_recent_pivots(candles, cfg.pivot_span, limit=120)
    if len(ph) < 3 or len(pl) < 2:
        return None
    # pick last three highs
    hL, hH, hR = ph[-3], ph[-2], ph[-1]
    if not (hH - hL >= cfg.hs_min_gap and hR - hH >= cfg.hs_min_gap):
        return None
    Lh = candles[hL].high
    Hh = candles[hH].high
    Rh = candles[hR].high
    ...
    if abs(Lh - Rh) / ((Lh + Rh) / 2) > cfg.hs_price_tolerance:
        return None
    # neckline: min of lows between L-H and H-R (approx)
...
def detect_inverse_head_shoulders(candles: List[Candle]) -> Optional[Dict]:
    ...
    if not (lH - lL >= cfg.hs_min_gap and lR - lH >= cfg.hs_min_gap):
        return None
    Ll = candles[lL].low
    Hl = candles[lH].low
    Rl = candles[lR].low
    if not (Hl < Ll and Hl < Rl):  # head is lowest
        return None
    if abs(Ll - Rl) / ((Ll + Rl) / 2) > cfg.hs_price_tolerance:
        return None
...
def breakout_seed_triangle(candles: List[Candle], tri: TriangleState, vol_ma_value: float) -> Optional[Dict]:
    i = len(candles) - 1
    last = candles[i]
    up = tri.up_m * i + tri.up_b
    lo = tri.lo_m * i + tri.lo_b
    if np.isnan(vol_ma_value):
        return None
...
def parse_ws_kline(msg: Dict) -> Optional[Tuple[str, str, Candle]]:
    try:
        ...
        k = msg["data"]["k"]
        if not k["x"]:
            return None  # only closed candle
        candle = Candle(
...
def stage_or_trigger(state: TFState, direction: str, idx: int, up_m: float, up_b: float, lo_m: float, lo_b: float, line_val: float, label: str) -> str:
    if cfg.confirm_bars > 0:
        state.pending_confirm = PendingConfirm(direction, cfg.confirm_bars, 0, up_m, up_b, lo_m, lo_b, label)
        return "STAGED"
    else:
        state.pending_retest = PendingRetest(direction, idx, up_m, up_b, lo_m, lo_b, label)
        return "TRIGGERED"
...
def process_triangle(sym: str, tf: str, st: TFState) -> Optional[str]:
    ...
    if tri is None:
        return None
    if not tri_contraction_ok(st.candles, tri, st.atr[i], st.atr):
        return None
    seed = breakout_seed_triangle(st.candles, tri, st.vol_ma[i])
    if seed is None:
        return None
    atr_recent = st.atr[i]
    if np.isnan(atr_recent) or not body_filters(seed, atr_recent):
        return None
...
def process_box(sym: str, tf: str, st: TFState) -> Optional[str]:
    i = len(st.candles) - 1
    if i < max(cfg.box_window + 5, cfg.vol_ma_window):
        return None
    rng = detect_box(st.candles)
    if not rng:
        return None
    seed = breakout_seed_box(st.candles, rng, st.vol_ma[i])
    if seed is None:
        return None
    atr_recent = st.atr[i]
    if np.isnan(atr_recent) or not body_filters(seed, atr_recent):
    ...
    if bar_confirms(pc.direction, st.candles[i].close, line_here):
        ...
        if pc.confirms_got >= pc.confirms_needed:
            ...
            if now - st.last_alert_time > 10:
                ...
                msg = (
                    f"*{sym.upper()} {tf}* — *BREAKOUT (confirmed)* `{pc.direction}` [{pc.label}]\n"
                ...
                await telegram(msg)
            st.pending_retest = PendingRetest(pc.direction, i, pc.up_m, pc.up_b, pc.lo_m, pc.lo_b, pc.label)
...
async def handle_retest(sym: str, tf: str, st: TFState):
    ...
    if pr.direction == "UP":
        ...
        if touched and confirm:
            ...
            msg = (
                f"*{sym.upper()} {tf}* — ✅ *Retest LONG Entry* [{pr.label}]\n"
        ...
        if touched and confirm:
            ...
            sl = lv["SL"]; L = leverage_for_loss(entry, sl)
            msg = (
                f"*{sym.upper()} {tf}* — ✅ *Retest SHORT Entry* [{pr.label}]\n"
                f"• Entry: `{entry:.6f}` (line `{line_here:.6f}`)\n"
                f"• SL: `{sl:.6f}`  (Lev≈`{L:.2f}x` to risk ~{cfg.loss_pct_at_sl*100:.0f}% at SL)\n"
                f"• TP_1.0R: `{lv['TP_1.0R']:.6f}`\n"
                f"• TP_2.0R: `{lv['TP_2.0R']:.6f}`"
...
def process_all_patterns(sym: str, tf: str, st: TFState) -> Optional[str]:
    # Try in order: Triangles -> Box -> Double -> H&S
...
async def main():
    # Start message with patterns list
    start_msg = (
        "🚀 Multi-Pattern Breakout Bot started\n"
    ...
    async for raw in multiplex_stream(SYMBOLS, TF_PRIORITY):
        ...
        # Then try to seed a new signal per priority
        for tf_c in TF_PRIORITY:
            tf_state = states[sym].tfs[tf_c]
